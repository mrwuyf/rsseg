# this work from the paper "Long-Range Correlation Supervision
# for  Land-Cover Classification From  Remote Sensing Images"
import torch
import torch.nn as nn
import torchvision
import torch.nn.functional as F
import numpy as np
from geoseg.models.backbones.resnet import resnet50
...
class SLCNet(nn.Module):
    ...
    def __init__(self, num_classes=2):
        ...
        down_blocks = []
        up_blocks = []
        self.input_block = nn.Sequential(*list(self.backbone.children()))[:3]

        self.input_pool = list(self.backbone.children())[3]
        for bottleneck in list(self.backbone.children()):
        ...
        side_outputs.append(Side_output(1024, num_classes))
        side_outputs.append(Side_output(512, num_classes))
        side_outputs.append(Side_output(256, num_classes))
        side_outputs.append(Side_output(128, num_classes))
        ...
        from geoseg.models.LovaszSoftmaxloss import lovasz_softmax
    ...
    def forward(self, x, label=None):
        ...
        for i in range(4):
            if label != None:
                ...
                label_ = F.interpolate(label_.float(), size=(side_outpus[i].size(2), side_outpus[i].size(3)),
                                       mode='nearest').long().cuda()
...
if __name__ == '__main__':
    ...
    model = SLCNet(num_classes=6).cuda()
